...
const config = {
  payaza: {
    apiKey: process.env.PAYAZA_API_KEY,        // e.g. "Payaza UFo3OC1..."
  ...
  paystack: {
    secretKey: process.env.PAYSTACK_SECRET_KEY, // e.g. "sk_live_..."
...
const payazaClient = axios.create({
  baseURL: PAYAZA_BASE,
  headers: {
    Authorization: config.payaza.apiKey,
...
async function withFallback(payazaFn, paystackFn, context = '') {
  try {
    const result = await payazaFn();
    return { ...result, provider: 'payaza' };
  } catch (err) {
...
// ═══════════════════════════════════════════════════════════════════════════════
async function createVirtualAccount({ amount, currency = 'NGN', customer, reference, type = 'Dynamic', expiresInMinutes = 30 }) {
  return withFallback(
    async () => {
...
//  CARD CHARGE
// ═══════════════════════════════════════════════════════════════════════════════
async function chargeCard({ amount, currency = 'NGN', customer, card, reference, description = 'Vaamoose payment', callbackUrl }) {
  return withFallback(
    async () => {
    ...
    async () => {
      // Paystack: tokenize card first via inline JS on frontend, then charge token
      // For direct API flow, use Paystack's charge endpoint
...
async function authorizeCard({ amount, currency = 'NGN', customer, card, reference }) {
  try {
    const { data } = await payazaClient.post('/card/card_charge/', {
      service_payload: {
        first_name: customer.firstName,
...
//  MOBILE MONEY
// ═══════════════════════════════════════════════════════════════════════════════
async function chargeMobileMoney({ amount, currency, customer, mobileNumber, bankCode, countryCode, reference }) {
  return withFallback(
    async () => {
      const { data } = await payazaClient.post(
...
// ═══════════════════════════════════════════════════════════════════════════════
async function bankEnquiry({ accountNumber, bankCode, currency = 'NGN' }) {
  return withFallback(
    async () => {
      const { data } = await payazaClient.post(
